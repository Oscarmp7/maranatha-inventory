    ...
    <form action={formAction} className="pf">
            ...
            {filtered.length === 0 ? (
              ...
              <button key={p.id} type="button" className="product-item" onClick={() => handleSelect(p)}>
                ...
                <span className="item-meta">
                  ...
                  {p.color ? ` · ${p.color}` : ''}
                  {' · '}
                  <span className={p.current_stock <= 0 ? 'stock-zero' : ''}>
                    {p.current_stock} {p.base_unit}s en stock
                  </span>
                </span>
              </button>
            ))}
      ...
      {selected && qty && parseFloat(qty) > 0 && !stockOk && (
        <div className="stock-warning">
          Stock insuficiente — disponible: {selected.current_stock} {selected.base_unit}s
        </div>
      )}
      ...
      <style>{`
        ...
        .calc-card {
          background: #f0fdf4; border: 1.5px solid #bbf7d0; border-radius: 12px; padding: 0.875rem 1rem;
