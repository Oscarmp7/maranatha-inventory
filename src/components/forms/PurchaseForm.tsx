...
export default function PurchaseForm({ products: initialProducts }: Props) {
  ...
  function handleCreateProduct() {
    startCreating(async () => {
      ...
      const result = await createProductQuick({
        ...
        units_per_package: newUnitsPerPkg,
      })
      if ('error' in result) {
  ...
  return (
    <form action={formAction} className="pf">
      ...
      <div className="field">
        ...
        {/* Botón añadir siempre visible cuando no hay producto seleccionado */}
        {!selected && !showCreate && (
        ...
        {selected && (
          <div className="selected-chip">
            ...
            <span className="chip-stock">Stock: {selected.current_stock} {selected.base_unit}s</span>
          </div>
        )}
      </div>

      {/* Mini-formulario: crear producto */}
      {showCreate && (
              ...
              {newPackageType && (
                <div className="field" style={{flex: 1}}>
                  <input
                    type="number"
                    className="input"
                    placeholder={`Unidades por ${newPackageType}`}
                    value={newUnitsPerPkg}
                    onChange={e => setNewUnitsPerPkg(e.target.value)}
                    min="1"
                    inputMode="numeric"
                  />
                </div>
      ...
      <div className="row-2">
        <div className="field">
          ...
          <div className="input-affixed">
            <input type="number" name="quantity_ordered" className="input" placeholder="0"
              ...
              min="0.01" step="any" inputMode="decimal" required />
        ...
        <div className="field">
          ...
          <div className="input-affixed">
            ...
            <input type="number" name="unit_cost" className="input input-pl" placeholder="0.00"
